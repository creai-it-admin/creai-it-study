...
import * as studyAccess from "../lib/study-access";
...
function load(file: string, dependencies: Record<string, unknown>) {
  ...
  runInNewContext(source, { module, exports: module.exports, URL, process: { env: {} },
    require: (name: string) => name in dependencies ? dependencies[name] : name === "@/lib/study-access" ? studyAccess : require(name),
  });
...
test("history query enforces current user, submitted status and closed session", async () => {
  const history = load("lib/history.ts", { "@/lib/prisma": { prisma: {
    submission: { findMany: async (args: any) => {
      ...
      assert.equal(args.where.formDef.session.weekNo.gt, 0);
      return [];
    } },
  } } });
  await history.getMySubmissions("current-user");
});
test("admin history retains rehearsal submissions", async () => {
  const history = load("lib/history.ts", { "@/lib/prisma": { prisma: {
    submission: { findMany: async (args: any) => {
      assert.equal(args.where.userId, "current-admin");
      assert.equal(args.where.formDef.session.weekNo, undefined);
      return [];
    } },
  } } });
  await history.getMySubmissions("current-admin", ["participant", "admin"]);
});
