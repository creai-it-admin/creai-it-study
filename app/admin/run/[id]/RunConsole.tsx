...
export function RunConsole({id}:{id:string}){
 ...
 return <div className="flex flex-col gap-5">
  <div className="flex flex-wrap items-center justify-between gap-3"><h1 className="text-xl font-semibold">{state.session.studyName} · {state.session.weekNo===0?'리허설':`${state.session.weekNo}주차`}</h1><Link href={`/admin/studies/${state.session.studyId}`} className="btn">회차 목록</Link></div>
  ...
  <div className="flex flex-wrap gap-2">{state.session.hasDeck&&<a className="btn" href={`/deck?session=${id}`} target="_blank" rel="noreferrer">HTML 장표 보기</a>}<a className="btn" href="/inclass" target="_blank" rel="noreferrer">인클래스</a><a className="btn" href="/inclass/shared" target="_blank" rel="noreferrer">제출물 보기</a>
   <button className="btn" disabled={busy||state.session.status!=='running'} onClick={()=>void sharing()}>{state.session.sharingOpen?'제출물 공유 닫기':'제출물 공유 열기'}</button><Link className="btn" href={`/sessions/${id}`}>세션 리포트</Link></div>
