...
export default async function AdminHome(){
 ...
 return <main className="mx-auto max-w-4xl px-5 py-8">
  <div className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-xl font-semibold">스터디 관리</h1><p className="mt-2 text-sm text-ink-2">스터디를 선택해 주차별 자료와 세션을 관리하세요.</p></div><Link className="btn btn-primary" href="/admin/studies/new">새 스터디 만들기</Link></div>
  <div className="grid gap-4 sm:grid-cols-2">{studies.map(study=>{
   ...
   return <Link className="card flex flex-col gap-4 p-6 hover:border-accent" href={`/admin/studies/${study.id}`} key={study.id}>
