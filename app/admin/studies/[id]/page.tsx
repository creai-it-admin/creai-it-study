...
export default async function StudyPage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="mx-auto max-w-4xl px-5 py-8">
  <Link className="text-sm text-accent-strong" href="/admin">← 스터디 목록</Link>
  <div className="mb-6 mt-4 flex flex-wrap items-center justify-between gap-4"><h1 className="text-xl font-semibold">{study.name}</h1><Link className="btn" href={`/admin/data?study=${study.id}`}>이 스터디 회차 결과</Link></div>
