...
export default async function SessionRecord({params}:{params:Promise<{id:string}>}){
 ...
 const session=await prisma.studySession.findFirst({where:{id,...sessionAccessWhere(user)},include:{study:true,recordingParts:{where:{uploadedAt:{not:null}},orderBy:[{recordedAt:'asc'},{id:'asc'}]}}});
 if(!session)notFound();
 const transcript=session.recordingParts.map((part,i)=>part.transcript?`[녹음 ${i+1}]\n${part.transcript}`:'').filter(Boolean).join('\n\n');
 return <>{await Header()}<main className="mx-auto flex max-w-6xl flex-col gap-5 px-5 py-8"><div className="flex flex-wrap items-center justify-between gap-3"><h1 className="text-xl font-semibold">{session.study.name} · {session.weekNo===0?'리허설':`${session.weekNo}주차`} 세션 리포트</h1><Link href="/home" className="btn">스터디 홈</Link></div>
  ...
  {session.deckPath&&<Link className="btn self-start" href={`/deck?session=${id}`}>이 회차 HTML 장표 보기</Link>}
 </main></>;
}
