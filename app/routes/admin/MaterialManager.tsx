...
export function MaterialManager({sessionId,materials,deckPath}:{sessionId:string;materials:MaterialItem[];deckPath:string|null}){
 ...
 return <div className="mt-1 border-t border-line pt-4">
  <div className="grid gap-4 sm:grid-cols-2">{(['education','presentation'] as const).map(kind=><div key={kind} className="inset min-w-0 p-4">
  ...
  {editing&&<form onSubmit={upload} className="mt-4 space-y-3 rounded-inner bg-accent-soft p-4" aria-label={`${materialLabels[editing.kind]} ${editing.id?'교체':'추가'}`}>
   <h3 className="text-sm font-semibold">{materialLabels[editing.kind]} {editing.id?'교체':'추가'}</h3>
   <label className="block text-sm">자료 제목<input className="field mt-1 block" required maxLength={120} value={title} disabled={busy} placeholder={editing.kind==='presentation'?'예: 재원 · AI 업무보고를 만들며 막혔던 지점':'예: 1주차 · 좋은 AI 활용'} onChange={e=>setTitle(e.target.value)}/></label>
