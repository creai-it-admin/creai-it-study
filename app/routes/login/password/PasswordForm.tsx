...
export function PasswordForm() {
  ...
  async function submit(event:FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.history.replaceState(null,'','/routes/login/password');
      setDone(true);
    }catch{setError('연결하지 못했습니다. 잠시 뒤 다시 시도해 주세요.');}
    finally{setBusy(false);}
  }
  if(done)return <div className="flex flex-col gap-4"><p role="status">비밀번호가 설정되었습니다.</p><Link className="btn btn-primary" href="/routes/login">로그인하기</Link></div>;
