...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const reason = formLockedReason(session.status, session.formDef?._count.submissions ?? 0, session.activityStatus);
    ...
    for (const [index, field] of input.fields.entries()) {
      const data = { question: field.question, stage:field.stage??"before", order: index + 1 };
