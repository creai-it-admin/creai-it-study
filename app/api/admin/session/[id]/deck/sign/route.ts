...
import { createDeckUploadUrl, MAX_DECK_BYTES } from "@/lib/storage";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { size, name } = (await req.json().catch(() => ({}))) as { size?: number; name?: string };
  ...
  if (!await prisma.studySession.findUnique({where:{id}})) return NextResponse.json({error:"not found"},{status:404});
  try {
    return NextResponse.json(await createDeckUploadUrl(id));
  } catch (e) {
