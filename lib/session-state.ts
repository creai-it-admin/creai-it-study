...
export async function getRunningSession(user:StudyViewer){
  return prisma.studySession.findFirst({where:{OR:[{status:'running'},{activityStatus:'open'}],...sessionAccessWhere(user)},orderBy:{date:'asc'},include:{study:true,_count:{select:{materials:true}}}});
}
