// Run from the app root: node docs/education/week1/build.cjs
...
function html(single=0){
 ...
 return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="CREAI+IT EDU Foundation 1주차 교육 초안"><title>CREAI+IT EDU · Foundation · Week 1${single?' · '+single:''}</title><style>${css}</style>${scripts}<script>${tokenScript}</script></head><body${single?` class="single" data-single="${single}"`:''}><main class="viewport" id="viewport" aria-label="1주차 슬라이드"><div class="stage" id="stage"><div id="slide-root"></div></div></main>${single?'':chrome}<script>${safe(read('viewer.js'))}</script></body></html>`;
}
for(let i=1;i<=20;i++)fs.writeFileSync(path.join(dir,`slide-${i}.html`),html(i));
fs.writeFileSync(path.join(dir,'index.html'),html());
console.log('Built 20 slides + self-contained index.html');
...
fs.writeFileSync(path.join(dir,'lecture-notes.md'),'# Foundation 1주차 · 강의 노트\n\n'+outline);
