// Run from the app root: node docs/education/week4/build.cjs
// JSX is precompiled for offline use; components.js remains the shared visual system.
const fs=require('node:fs'),path=require('node:path'),ts=require('typescript');
const dir=__dirname;
const read=name=>fs.readFileSync(path.join(dir,name),'utf8');
const compile=name=>{const r=ts.transpileModule(read(name),{reportDiagnostics:true,compilerOptions:{jsx:ts.JsxEmit.React,target:ts.ScriptTarget.ES2020}});if(r.diagnostics?.length)throw Error(ts.formatDiagnosticsWithColorAndContext(r.diagnostics,{getCanonicalFileName:f=>f,getCurrentDirectory:()=>dir,getNewLine:()=>String.fromCharCode(10)}));return r.outputText;};
fs.writeFileSync(path.join(dir,'src/slides.jsx'),'(() => {\nconst DATA='+read('slides.json')+';\n'+read('src/render.jsx'));
...
function html(single=0){
 ...
 return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="CREAI+IT EDU Foundation 4주차 교육 초안"><title>CREAI+IT EDU · Foundation · Week 4${single?' · '+single:''}</title><style>${css}</style>${scripts}<script>${tokenScript}</script></head><body${single?` class="single" data-single="${single}"`:''}><main class="viewport" id="viewport" aria-label="4주차 슬라이드"><div class="stage" id="stage"><div id="slide-root"></div></div></main>${single?'':chrome}<script>${safe(read('viewer.js'))}</script></body></html>`;
}
for(let i=1;i<=JSON.parse(read('slides.json')).length;i++)fs.writeFileSync(path.join(dir,`slide-${i}.html`),html(i));
fs.writeFileSync(path.join(dir,'index.html'),html());
console.log(`Built ${JSON.parse(read('slides.json')).length} slides + self-contained index.html`);

const vm=require('node:vm'),context={window:{}};
vm.runInNewContext(slides,context);
const outline=context.window.OT_SLIDES.map((s,i)=>`## ${i+1}. ${s.title}\n\n${s.note}\n`).join('\n');
fs.writeFileSync(path.join(dir,'lecture-notes.md'),'# Foundation 4주차 · 강의 노트\n\n'+outline);
